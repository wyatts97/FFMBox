...
export const FloatingSettings: React.FC = () => {
    ...
    if (open) {
      ...
      fetch('/api/settings')
        ...
        .then(data => {
          ...
          setOutputDir(data.outputDir ?? '/app/output');
          setAutoDownload(data.autoDownload ?? false);
        })
      ...
      fetch('/api/history')
        .then(res => res.json())
        .then(data => setConversionHistory(data))
        .catch(err => console.error("Failed to fetch history:", err));
    }
  }, [open]);

  // Update settings in backend
  const updateSettings = (updates: Partial<{retentionPeriod: string, outputDir: string, autoDownload: boolean}>) => {
    fetch('/api/settings', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        autoDownload: updates.autoDownload ?? autoDownload
      })
    })
      .then(res => res.json())
      .then(data => {
  ...
  const handleCleanup = async () => {
    toast({ title: 'Initiating Cleanup...', description: 'Requesting server to clean up old files.' });
    try {
      ...
      toast({ title: 'Cleanup Failed', description: 'Could not initiate the cleanup process.', variant: 'destructive' });
    }
  };
  ...
  const handleClearHistory = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: 'History Cleared', description: 'Your conversion history has been cleared.' });
      } else {
        throw new Error('Failed to clear history');
      }
    } catch (error) {
      toast({ title: 'Error', description: 'Could not clear conversion history.', variant: 'destructive' });
    }
  };
