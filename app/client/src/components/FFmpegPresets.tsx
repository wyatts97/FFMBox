...
import { 
  type AnyPreset, 
  type BasePreset as FFMpegBasePreset, 
  type PresetCategory, 
  type PresetOptions, 
  type PresetState,
  isBasePreset,
...
export const FFmpegPresets: React.FC<FFmpegPresetsProps> = ({
  ...
  // Save preset safely with proper error handling
  const savePresetSafe = useCallback(async (preset: Omit<CustomPreset, 'id' | 'category'>): Promise<CustomPreset> => {
    try {
      // Create a new preset with required properties
      const newPreset: Omit<CustomPreset, 'category'> = {
        ...preset,
        id: `temp-${Date.now()}`, // Temporary ID, will be replaced by the service
  ...
  const handleSaveNewPreset = useCallback(async (): Promise<CustomPreset> => {
    if (!presetName.trim()) {
      throw new Error('Preset name cannot be empty');
    }

    const newPreset = await savePresetSafe({
    ...
    return newPreset;
  }, [presetName, customCommand, presetDescription, savePresetSafe, updateState]);
  ...
  const handleTabChange = (value: string) => {
    updateStateSafe(prevState => ({
      ...prevState,
      activeTab: value as 'presets' | 'custom'
    }));
  };
  ...
  const handlePresetDescriptionChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    updateStateSafe(prevState => ({
      ...prevState,
      presetDescription: e.target.value
    }));
  };
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {disabled && (
        <div className="bg-yellow-100 dark:bg-yellow-900 border border-yellow-400 border-solid dark:border-yellow-700 text-yellow-800 dark:text-yellow-200 px-4 py-3 rounded relative mb-4" role="alert">
          <strong className="font-bold">No files uploaded! </strong>
          <span className="block sm:inline">Please upload at least one file before selecting a preset.</span>
        </div>
      )}
      <Tabs 
        value={activeTab} 
